...
def coletar_links_pagina(bot, medicamento, localizacao, farmacia_nome, limite=10):
    ...
    for item in links:
        ...
        try:
            ...
            if titulo and link and medicamento.lower() in titulo.lower():
...
def buscar_medicamento(medicamento, localizacao):
    ...
    try:
        print(f"Pesquisando {medicamento} na Drogasil...")

        url_drogasil = (
            "https://www.drogasil.com.br/search?w="
            + quote_plus(medicamento)
        )

        bot.browse(url_drogasil)
        time.sleep(8)

        resultados_drogasil = coletar_links_pagina(
            bot=bot,
            medicamento=medicamento,
            localizacao=localizacao,
            farmacia_nome="Drogasil",
            limite=10
        )

        resultados.extend(resultados_drogasil)

    except Exception as erro:
        print("Erro na Drogasil:", erro)
...
def montar_resposta(medicamento, localizacao, resultados):
    ...
    for item in resultados[:10]:
        resposta += (
            ...
            f"Predição inicial/Relevância: {item['relevante']}\n"
            f"Link: {item['link']}\n\n"
        )
